...
SELECT m.id AS municipio_id, m.nombre AS municipio_nombre, AVG(e.salario) AS salario_promedio
FROM empleados e
JOIN sucursal s ON e.sucursalid = s.id
JOIN municipio m ON s.municipioid = m.id
...
SELECT d.id AS departamento_id, d.nombre AS departamento_nombre, COUNT(c.cliente_id) AS total_clientes
FROM clientes c
JOIN municipio m ON c.municipioid = m.id
JOIN departamento d ON m.depid = d.id
...
FROM municipio m
WHERE m.id NOT IN (
    SELECT s.municipioid
    FROM sucursal s
    WHERE s.municipioid IS NOT NULL
);
